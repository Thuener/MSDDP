...
function samplestp1(dS::SDPData, dF::ARData, Q_l::Array{Float64,1}, z_l::Array{Float64,1}, z_t::Float64)
  e = lhsnorm(zeros(dS.N+1), dF.Σ, dS.S, rando=false)'
  r_s = Array(Float64, dS.N, dS.S)
  Q_s = Array(Float64, dS.S)
  z_tp1 = Array(Float64, dS.S)
  for s = 1:dS.S
    ρ =  dF.a_r +dF.b_r*z_t + e[1:dS.N,s]
    # Transform ρ = ln(r) in return (r)
    r_s[:,s] = exp(ρ)-1
    # Discount risk free rate
    r_s[:,s] -= dF.r_f
    z_tp1[s] = dF.a_z[1] +dF.b_z[1]*z_t + e[dS.N+1,s]
  end
  knots = (z_l,)
  itp = interpolate(knots, Q_l, Gridded(Linear()))
  Q_s = itp[z_tp1]
  return Q_s, r_s
end
...
function createmodel(dS::SDPData, p::Array{Float64,1}, r::Array{Float64,2}, Q_tp1::Array{Float64,1}; LP::Int64=2)
  ...
  risk =  @constraint(Q,-(z - sum{p[s]*y[s] , s = 1:dS.S}/(1-dS.α)) <= dS.γ).idx #TODO igualar ao paper
...
function forward(dS::SDPData, dF::ARData, Q_l::Array{Float64,2}, z_l::Array{Float64,1}, z_t::Array{Float64,1},
    rets_t::Array{Float64,2})
  W = 1.0
  p_s = ones(dS.S)*1/dS.S
  all = Array(Float64,dS.N+1, dS.T-1)
  for t = 1:dS.T-1
    Q_s, r_s = samplestp1(dS, dF, vec(Q_l[t+1,:]), z_l, z_t[t])
...
function backward(dF::ARData, dS::SDPData, z_l::Array{Float64,1})
  ...
  Q_l[dS.T,:] = ones(Float64,dS.L)
  p_s = ones(dS.S)*1/dS.S
  for t = dS.T-1:-1:1
    e = lhsnorm(zeros(dS.N+1), dF.Σ, dS.S, rando=false)'
    info(" t = $t")
    for l = 1:dS.L
      Q_s, r_s = samplestp1(dS, dF, vec(Q_l[t+1,:]), z_l, z_l[l])
      Q = createmodel(dS, p_s, r_s, Q_s)
      status = solve(Q)
      if status ≠ :Optimal
        writeLP(Q,"prob.lp")
