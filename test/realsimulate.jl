...
LB, UB, LB_c, AQ, sp, list_α, list_β = sddp(dH, dM)

k_test = predict(model,ret_test)
ret_test = exp(ret_test)-1
x, x0 = simulatesw(dH, dM, AQ, sp, list_α, list_β, ret_test', k_test)
...
x_sw, x0_sw = simulate(dH, dM, AQ, sp, list_α, list_β, ret_test', k_test)
